...
#include "C:\Users\anton\source\repos\ThreadsSync\ThreadsSync\MainProcess.h"
...
static DWORD WINAPI marker_thread_test(LPVOID arg) {
	WaitForSingleObject(testProcess->start_events[(int)arg - 1], INFINITE);
	...
	while (true) {
		...
		if (testProcess->arr[index] == 0) {
			...
			marked_elements = 0;
			ResetEvent(testProcess->start_events[marker_id - 1]);
			SetEvent(testProcess->terminate_events[marker_id - 1]);
			LeaveCriticalSection(&testProcess->cs);
			WaitForSingleObject(testProcess->start_events[marker_id - 1], INFINITE);
			EnterCriticalSection(&testProcess->cs);
...
namespace ThreadsSynctest
{
	TEST_CLASS(ThreadsSynctest)
	{
	public:
		...
		TEST_METHOD(CreateEvents)
		{
			...
			Assert::AreEqual(static_cast<size_t>(num_threads), mainProcess.start_events.size());
			for (int i = 0; i < num_threads; ++i) {
				Assert::IsNotNull(mainProcess.start_events[i]);
				Assert::IsNotNull(mainProcess.terminate_events[i]);
			}
		}
